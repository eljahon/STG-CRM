...
export const CustomMultiSelect: React.FC<CustomMultiSelectProps> =(props) => {
    const { url,param, form, field,optionsProp,  optionLabel='name',placeholder,optionValue='id', customFilter, isLoading } = props;
    ...
    return (
        <div className="w-full">
            {isLoading ? <Skeleton width="100%" height="3rem" /> :<><MultiSelect
                loading={loading}
                panelHeaderTemplate={() => (
                    <div className="w-full">
            <span className="p-input-icon-right w-full p-2" style={{width: '100%'}}>
              <InputText
              ...
              <i className="pi pi-search" />
                ...
                value={field?.value}
                onChange={(e: MultiSelectChangeEvent) => form.setFieldValue(field?.name, e.value)
                }
